...
def get_school_special_plan(db):
    columns = [
        'batch_type', 'year', 'school_id', 'special_id', 'type', 'batch', 'num', 'zslx', 'province',
        'length', 'tuition', 'spe_id', 'info', 'special_group', 'first_km', 'sp_type', 'sp_fxk',
        'sp_sxk', 'sp_info', 'sp_xuanke', 'level1_name', 'level2_name', 'level3_name', 'level1',
        'level2', 'level3', 'spcode', 'spname', 'local_batch_name', 'zslx_name', 'sg_fxk', 'sg_sxk',
        'sg_type', 'sg_name', 'sg_info', 'sg_xuanke'
    ]

    years = [2019, 2020, 2021, 2022, 2023, 2024]
    school_ids = get_all_school_ids(db)
    table_name = 'school_special_plan'

    for school_id in school_ids:
        for year in years:
            url = f"https://static-data.gaokao.cn/www/2.0/schoolspecialplan/{school_id}/{year}/35.json"
